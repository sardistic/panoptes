...
def _conn() -> sqlite3.Connection:
    ...
    if not _ready:
        ...
        CREATE TABLE IF NOT EXISTS looks (
            ...
            weather TEXT, cameras TEXT, camera_ids TEXT, counts TEXT, sky TEXT, street TEXT
        );
        ...
        if "street" not in cols:
            c.execute("ALTER TABLE looks ADD COLUMN street TEXT")
        c.commit()
        _ready = True
    return c
...
def record(bounds: dict, focus: str, out: dict, counts: dict | None = None) -> str:
    ...
    with _lock:
        ...
        c.execute("INSERT INTO looks VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (
            ...
            json.dumps(out.get("sky") or {}), json.dumps(out.get("street") or [])))
        c.commit()
    return uid
...
def _row(r) -> dict:
    return {"uid": r[0], "ts": r[1], "bounds": {"south": r[2], "north": r[3], "west": r[4], "east": r[5]},
            ...
            "street": json.loads((r[14] if len(r) > 14 else None) or "[]")}
